...
Inputs (from peak_to_parquets_v1.py outputs):
...
# ------------------------- Functionally selected features filtering -------------------------------
def fdr_filter_functional_results(
    df: pd.DataFrame,
    viruses: Sequence[str],
    modalities: Sequence[str],
    polarities: Sequence[str],
    q: float,
) -> pd.DataFrame:
    """
    FDR-filter functional selection results within each (virus, modality, polarity) block.
    ...
    viruses : sequence of str
        Subset V ⊂ {"CHIKV", "DENV", "ZIKV"} to include.
    modalities : sequence of str
        Subset M ⊂ {"metabolite", "lipid"} to include.
    polarities : sequence of str
        Subset P ⊂ {"+", "-"} to include.
    q : float
    ...
    -------
    pd.DataFrame
        Filtered DataFrame containing only rows with:
          - virus ∈ V, modality ∈ M, polarity ∈ P, and
          - feature passing Benjamini–Hochberg FDR at level q
            within its (virus, modality, polarity) block.

        Column set is unchanged from the input df.
    """
    required_cols = {"feature_id", "modality", "polarity", "virus", "weighted_l2", "p_value", "n_NaNs"}
    missing = required_cols - set(df.columns)
    if missing:
        raise ValueError(f"Input df is missing required columns: {sorted(missing)}")

    # Restrict to requested (virus, modality, polarity) combinations
    mask_query = (
        df["virus"].isin(viruses)
        & df["modality"].isin(modalities)
        & df["polarity"].isin(polarities)
    )
    df_sub = df.loc[mask_query].copy()

    if df_sub.empty:
        # Nothing matches the query; return empty with same columns
        return df_sub

    # Global mask of which rows (in df_sub) pass FDR in their group
    keep_mask = pd.Series(False, index=df_sub.index)

    # Iterate over each (virus, modality, polarity) triple
    for v in viruses:
        for m in modalities:
            for p in polarities:
                block_mask = (
                ...
                idx = df_sub.index[block_mask]
                if idx.size == 0:
                    continue

                pvals = df_sub.loc[idx, "p_value"].to_numpy(dtype=float)

                # Ignore NaN p-values (they can never be significant)
                finite_mask = np.isfinite(pvals)
                if not np.any(finite_mask):
                    continue

                idx_finite = idx[finite_mask]
                pvals_finite = pvals[finite_mask]

                # Benjamini–Hochberg procedure within this block
                m_tests = pvals_finite.size
                order = np.argsort(pvals_finite)
                p_sorted = pvals_finite[order]

                # BH thresholds: (i/m) * q for i=1..m
                ranks = np.arange(1, m_tests + 1, dtype=float)
                thresholds = (ranks / m_tests) * q
                ...
                k_star = np.max(np.where(below)[0])  # 0-based index
                # Indices (within pvals_finite) that pass BH
                passed_local = order[: k_star + 1]
                passed_idx = idx_finite[passed_local]

                keep_mask.loc[passed_idx] = True

    # Return only rows that passed FDR within their (v, m, p) group
    return df_sub.loc[keep_mask]
